...
function sessionForClient(array $row): array {
    ...
    $billing=(string)($row['billing_type']??'hourly'); $memberName=trim((string)($row['member_name']??'')); $packageName=trim((string)($row['package_name']??''));
    $billingInfo=$billing==='member'&&$memberName!==''?'Member Pass: '.$memberName:($billing==='hourly'?'OPEN':$packageName);
    $startMs=(int)dbMs($row['start_time']); $endMs=dbMs($row['end_time']); $pauseMs=dbMs($row['pause_time']); $pausedTotal=max(0,(int)$row['paused_minutes'])*60000;
...
try {
    ...
    try {
        ...
        foreach($data['consoles'] as $console){
            ...
            $s=$console['session']??null; if(!is_array($s)) continue; $billing=(string)($s['billingInfo']??'OPEN'); $memberId=null;
            ...
            $billingType=$memberId?'member':($billing==='OPEN'?'hourly':'package'); $startMs=(int)($s['startTime']??0); $start=dtMs($startMs) ?: date('Y-m-d H:i:s'); $end=dtMs($s['endTime']??null); $pausedMs=max(0,(int)($s['totalPausedDuration']??0)); $pausedMinutes=(int)round($pausedMs/60000); $pauseTime=dtMs($s['pauseTime']??null); $duration=(int)($s['totalPaketMinutes']??0); $cost=(float)($s['totalPaketCost']??0);
            ...
            if($existingId) $update->execute([$memberId,$packageId,$billingType,$dbStatus,$start,$end,$duration,$pausedMinutes,$cost,$notes,$orders,$pauseTime,$existingId]); else $insert->execute([$consoleId,$memberId,$packageId,$billingType,$dbStatus,$start,$end,$duration,$pausedMinutes,$cost,$notes,$orders,$pauseTime]);
            $setConsoleStatus->execute([$status==='paused'?'paused':'playing',$consoleId]);
        }
        $pdo->commit();
    } catch(Throwable $e){ if($pdo->inTransaction())$pdo->rollBack(); throw $e; }
    response(['success'=>true,'data'=>fetchServerTruth($pdo),'mode'=>'master']);
