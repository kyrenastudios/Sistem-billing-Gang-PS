document.addEventListener('DOMContentLoaded', () => {
    ...
    function createSearchableSelect(selectElement) {
        ...
        input.addEventListener('input', () => {
            ...
            const prev = dropdown.querySelector('.highlighted');
        ...
        input.addEventListener('focus', () => {
            ...
            if (sel && !input.value && (sel.value || '').toString().trim() !== '') {
    ...
    cartList.addEventListener('click', e => {
        if (!e.target.matches('.btn-delete-order')) return;
    ...
    loadMenuItems().then(() => {
        try {
            createSearchableSelect(selectMenu);
        } catch (e) {
            console.error('Gagal membuat pencarian menu:', e);
        }
    });
});